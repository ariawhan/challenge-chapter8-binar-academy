...
describe('POST /v1/cars/:id/rent', () => {
  ...
  it('should response with 401 as status code (userAdmin cannot be update car)', async () => await request(app)
    .post('/v1/auth/login') // request api login
    .set('Content-Type', 'application/json')
    // need email and password for login userCustomer
    .send({ email: userRentAdmin.email, password })
    .then(async (res) => {
      await request(app)
  ...
  it('should response with 404 as status code (Cars not found for rent)', async () => await request(app)
    .post('/v1/auth/login') // request api login
    .set('Content-Type', 'application/json')
    // need email and password for login userCustomer
    .send({ email: userRentCustomerRent.email, password })
    .then(async (res) => {
      await request(app)
  ...
  it('should response with 201 as status code (Suksess Rent Cars)', async () => await request(app)
    .post('/v1/auth/login') // request api login
    .set('Content-Type', 'application/json')
    // need email and password for login userCustomer
    .send({ email: userRentCustomerRent.email, password })
  ...
  it('should response with 422 as status code (al)', async () => await request(app)
    .post('/v1/auth/login') // request api login
    .set('Content-Type', 'application/json')
    // need email and password for login userCustomer
    .send({ email: userRentCustomerRent.email, password })
  ...
  it('should response with 500 as status code (invalid format)', async () => await request(app)
    .post('/v1/auth/login') // request api login
    .set('Content-Type', 'application/json')
    // need email and password for login userCustomer
    .send({ email: userRentCustomerRent.email, password })
